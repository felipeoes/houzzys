...
const FAVORITE_KEY = '@Houzzys:Favorites';
...
export const saveHouseAsFavorite = async (
  ...
  if (savedFavorites) {
    if (savedFavorites.find(h => h === houseId)) {
      return { error: 'You have already favorited this home.' };
    }
...
export const getIfHouseIsFavorite = async (
  ...
  const savedFavorites = await getData(FAVORITE_KEY);
  if (savedFavorites) {
    return savedFavorites.some(h => h === houseId);
  }
  return false;
};
...
export const removeHouseAsFavorite = async (
  ...
  if (savedFavorites) {
    ...
    return {
      error: 'You have not favorited this home yet.',
    };
  }
